    ...
    if (open) {
      if (isEditMode && item) {
        form.reset({
          name: item.name,
          price: item.price,
          // Assuming purchase price could be different, but for now we don't store it on the item model
          // You might want to add `purchasePrice` to your `Item` type and `data.ts`
  ...
  const onSubmit = async (data: ItemFormValues) => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: `Failed to ${isEditMode ? 'update' : 'create'} item: ${message}`,
      });
    }
  };
